...
} from '../components/PlaylistFeed.jsx';
...
function PlaylistPage() {
    ...
    const fetchCurrentPlaylist = useCallback(
        async (reload = false) => {
            if (!user) return;
    ...
    const fetchNextItems = useCallback(
        async (reset = false, fetchAmount = FETCH_AMOUNT) => {
            if (!hasMoreItems) {
                return;
            }
            ...
            if (reset) {
                ...
                itemsToFetch = playlist.items.slice(
                    playlistItems.length,
                    playlistItems.length + fetchAmount
                );
            }
            ...
            if (reset) {
                ...
                setHasMoreItems(
                    (playlistItems.length, nextItems.length) !==
    ...
    const saveRearrangedItems = useCallback(async () => {
        ...
        try {
            const response = await playlistService.rearrangePlaylistItems(
                playlist.$id,
                rearrangedIDs
            );

            const newPlaylistItems = await getMusicDetails(
                response.items.slice(0, rearrangedIDs.length),
                false,
                rearrangableItems
            );
    ...
    useEffect(() => {
        ...
        fetchNextItems(
            true,
            playlistItems?.length ? playlistItems.length : undefined
        );
    }, [playlist]);
    ...
    useEffect(() => {
        playlistService.isPlaylistOwned(user?.$id, documentId).then((ans) => {
            setIsOwned(ans);
        });
    }, [documentId, user]);
    ...
    if (playlist.notFound) {
        return (
            <div className='w-full pt-56'>
                <p className='text-8xl text-center select-none'>404</p>
                <p className='text-5xl text-center select-none'>Not Found</p>
