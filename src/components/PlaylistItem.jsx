    ...
    return (
        ...
        >
            <div className='flex-none flex justify-center items-center w-[3ch] text-stone-300 font-semibold'>
                {isRearrangable ? (
                    <Icon iconSrc={dragIcon} className='p-0.5' />
                ) : (
                    index
                )}
            </div>
            ...
            <div className='flex-1 flex flex-col justify-between items-start py-1.5'>
                ...
                <div className='flex justify-start items-center'>
                    <SmallP>{item.channelTitle}</SmallP>

                    <SmallP className='mx-1'>·</SmallP>

                    <SmallP>{convertUploadTimeFormat(item.uploadTime)}</SmallP>
                </div>

                <SmallP>{convertDurationFormat(item.duration)}</SmallP>
            </div>
...
function SmallP({ children, className }) {
    return (
        <p
            className={`text-[13px]/[13px] text-stone-400 line-clamp-1 break-all ${className}`}
        >
            {children}
        </p>
    );
