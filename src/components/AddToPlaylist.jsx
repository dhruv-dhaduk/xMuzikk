...
function AddToPlaylist({ music, callback }) {
    ...
    const handleAddToPlaylist = useCallback(async (playlist) => {
        try {
            const response = await playlistService.addToPlaylist(playlist?.$id, music?.id);
    ...
    useEffect(() => {
        ...
        const getUserPlaylists = async () => {
            const playlistsDocumentIDs = await playlistService.getOwnedPlaylists(user?.$id);
            const playlists = await playlistService.fetchPlaylists(playlistsDocumentIDs, ['$id', 'title']);
